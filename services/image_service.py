...
def flatten_scene_images(scene_data, all_drawings):
    ...
    for idx, scene in enumerate(scene_data):
        ...
        try:
            ...
            bg_full_path = (
                os.path.join(Config.BASE_DIR, bg_path.replace("/", os.sep))
                if bg_path
                else None
            )
            ...
            matched = [
                ...
                if d.get("korean") and _contains(text, d["korean"])
            ]
            if not matched:
...
def generate_moral_collage(moral_text, all_drawings):
    ...
    """
    try:
        sketchbook_path = os.path.join(
            Config.BASE_DIR, "static", "images", "sketchbook.png"
        )
        ...
        for d in all_drawings:
            ...
            d_img = d_img.rotate(
                random.randint(-15, 15), expand=True, resample=Image.BICUBIC
            )
...
def _get_pipe():
    ...
    try:
        ...
        if free_vram < 1.5:
            print(
                f"[로컬 SD] VRAM 여유 부족 ({free_vram:.1f}GB) → Pollinations.ai 폴백 모드로 전환"
            )
            _pipe_load_failed = True
            return None
        ...
        try:
            ...
            _pipe.unet = torch.compile(
                _pipe.unet, mode="reduce-overhead", backend="tensorrt"
            )
...
def generate_reference_image(
    korean_word: str, english_word: str, image_prompt: str = None
) -> str | None:
    """
