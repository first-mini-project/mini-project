...
(function () {
  ...
  function getBounds() {
    ...
    const topY = subtitle ? subtitle.getBoundingClientRect().bottom + 10 : 160;
    ...
    if (bottomY - topY < 150)
      return { top: topY, bottom: H - 70, left: 8, right: W - 8 };
  ...
  const physCards = origCards.map((orig) => {
    ...
    Object.assign(el.style, {
      ...
      top: "0",
      width: CARD_W + "px",
      boxSizing: "border-box", // 패딩/보더 포함 정확히 CARD_W
      margin: "0",
      cursor: "pointer",
      pointerEvents: "auto",
      opacity: "0",
    ...
    el.addEventListener("click", () => {
      if (state.wasJustDragged) {
        state.wasJustDragged = false;
        return;
      }
      if (floatMode) toggle(state.id);
    });
    el.addEventListener("mouseenter", () => {
      if (!state.dragging) state.hovered = true;
    });
    el.addEventListener("mouseleave", () => {
      if (!state.dragging) state.hovered = false;
    });
  ...
  origCards.forEach((card) => {
    card.addEventListener("click", () => {
      if (!floatMode) toggle(card.dataset.id);
    });
  });
  ...
  function toggle(id) {
    ...
    physCards.forEach((s) =>
      s.el.classList.toggle("selected", selected.has(s.id)),
    );
    origCards.forEach((c) =>
  ...
  function updateUI() {
    ...
    [
      ...physCards.map((s) => ({ el: s.el, id: s.id })),
      ...origCards.map((c) => ({ el: c, id: c.dataset.id })),
    ].forEach(({ el, id }) =>
      el.classList.toggle("disabled", count >= MAX && !selected.has(id)),
    );
  ...
  function initPositions() {
    ...
    physCards.forEach((s) => {
      ...
      do {
        ...
        placed.some(
          (p) =>
            Math.hypot(p.cx - (x + CARD_W / 2), p.cy - (y + CARD_H / 2)) <
            MIN_D,
        )
      );
  ...
  function setMode(mode) {
    ...
    if (btnFloat) {
      btnFloat.className = floatMode
        ? "btn btn-pink btn-sm"
        : "btn btn-yellow btn-sm";
      btnFloat.style.cssText = "";
    }
    if (btnGrid) {
      btnGrid.className = floatMode
        ? "btn btn-yellow btn-sm"
        : "btn btn-pink btn-sm";
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    if (dragMoved) {
      ...
      dragging.x = Math.max(
        b.left,
        Math.min(e.clientX - dragging.dragOffsetX, b.right - CARD_W),
      );
      dragging.y = Math.max(
        b.top,
        Math.min(e.clientY - dragging.dragOffsetY, b.bottom - CARD_H),
      );
    }
  });
  ...
  window.addEventListener("mouseup", () => {
    ...
    if (dragging.dragging) {
      ...
      const velScale = 0.45;
      dragging.vx = Math.max(
        -MAX_SPEED,
        Math.min((lastMX - prevMX) * velScale, MAX_SPEED),
      );
      dragging.vy = Math.max(
        -MAX_SPEED,
        Math.min((lastMY - prevMY) * velScale, MAX_SPEED),
      );
  ...
  function tick() {
    ...
    physCards.forEach((s) => {
      ...
      if (spd < 0.3) {
        const ang =
          spd > 0.01 ? Math.atan2(s.vy, s.vx) : Math.random() * Math.PI * 2;
        s.vx = Math.cos(ang) * SPEED;
        s.vy = Math.sin(ang) * SPEED;
      }
      // 최대 속도 제한
    ...
    physCards.forEach((s) => {
      ...
      s.el.style.boxShadow = s.dragging
        ...
        : active
          ...
          : selected.has(s.id)
            ? "0 0 0 4px rgba(255,107,157,0.35)"
            : "";
    });
  ...
  function openProtagonistModal(drawingIds) {
    ...
    drawingIds.forEach((id) => {
      ...
      lbl.textContent = label
        ? label.textContent.replace(/[^\uAC00-\uD7A3a-zA-Z0-9 ]/g, "").trim()
        : "";
      card.appendChild(lbl);

      card.addEventListener("click", () => {
        protagonistCards
          .querySelectorAll(".protagonist-card")
