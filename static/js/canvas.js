(function () {
  ...
  function showLevelUpModal(info) {
    ...
    document.getElementById('level-up-badge').textContent = info.badge.split(' ')[0];
  ...
  function spawnConfetti() {
    const container = document.getElementById('praise-confetti') || document.getElementById('level-up-modal');
    if (!container) return;
    ...
    helpBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (data.success && data.image) {
          if (exampleImage) { exampleImage.src = data.image; exampleImage.style.display = 'block'; }
        } else {
          if (examplePlaceholder) {
            examplePlaceholder.innerHTML = '<span class="example-placeholder-icon">😅</span><span>' + (data.error || 'AI 이미지 생성 불가') + '</span>';
            examplePlaceholder.style.display = 'flex';
          }
        }
      } catch (err) {
        if (examplePlaceholder) {
          examplePlaceholder.innerHTML = '<span class="example-placeholder-icon">😅</span><span>오류가 발생했어요</span>';
          examplePlaceholder.style.display = 'flex';
        }
      } finally {
