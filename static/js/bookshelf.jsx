    ...
    pages: [
      {
        page: 1,
        content_kr:
          "옛날 옛날 아주 먼 옛날, 깊은 숲속에 호기심 많은 아기 토끼가 살고 있었어요. 아기 토끼는 매일매일 궁금한 것이 가득했어요.",
      },
      {
        page: 2,
        content_kr:
          "어느 날 아기 토끼는 커다란 무지개를 보았어요. '와, 저 무지개는 어디로 가는 걸까?' 아기 토끼는 무지개를 따라가 보기로 결심했어요.",
      },
    ],
  },
  {
    ...
    pages: [
      ...
      {
        page: 2,
        content_kr: "요정은 잃어버린 도토리를 찾아 숲속을 헤매기 시작했어요.",
      },
    ],
  },
];
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: styleConf.spine }}
      ></div>
      ...
      {book.coverImage ? (
        <div className="book-cover-image-wrapper">
          <img
            ...
            alt=""
            className="book-cover-image-preview"
          />
        </div>
      ) : (
        <div
          className="book-decoration"
          style={{ fontSize: styleConf.shapeSize, color: styleConf.shapeColor }}
        >
          {styleConf.shape}
        </div>
      )}
    ...
    >
      <div className="shelf-section">
        ...
        <div className="storage-section">
          <div className="storage-cubby">
            <div className="bin">
              <div className="bin-handle"></div>
            </div>
          </div>
          <div className="storage-cubby">
            <div className="bin">
...
function StoryModal({ book, idx, onClose, onReadComplete }) {
  ...
  function getDrawingSrc(d) {
    ...
    return (
      nukkiMap[d.id] ||
      (d.file_path ? "/" + d.file_path.replace(/^\/+/, "") : null)
    );
  }
  ...
  const pageAssignments = useMemo(() => {
    ...
    const result = book.pages.map((page) => {
      ...
      let primary = null,
        secondary = null,
        tertiary = null;
      if (page.primaryDrawingId)
        primary = drawings.find((d) => d.id === page.primaryDrawingId) || null;
      if (page.secondaryDrawingId)
        secondary =
          drawings.find((d) => d.id === page.secondaryDrawingId) || null;
    ...
    for (const d of unshown) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        if (!result[i].secondary) {
          result[i].secondary = d;
          placed = true;
          break;
        }
      }
  ...
  function renderLeftPage() {
    ...
    if (isMoralPage) {
      ...
      return (
        ...
        >
          <img
            src="/static/images/sketchbook.png"
            alt="스케치북"
            onLoad={(e) => {
    ...
    if (primary && !secondary) {
      drawingNodes = pSkyClass
        ? skyStage(primary, pSkyClass, pScale)
        : groundStage(primary, "center", pScale);
    } else if (primary && secondary) {
      if (!pSkyClass && sSkyClass)
        drawingNodes = (
          <>
            {groundStage(primary, "center", pScale)}
            {skyStage(secondary, sSkyClass, sScale)}
          </>
        );
      else if (pSkyClass && !sSkyClass)
        drawingNodes = (
          <>
            {groundStage(secondary, "center", sScale)}
            {skyStage(primary, pSkyClass, pScale)}
          </>
        );
      else if (!pSkyClass && !sSkyClass)
        drawingNodes = (
          <>
            {groundStage(primary, "left", pScale)}
            {groundStage(secondary, "right", sScale)}
          </>
        );
      else {
        ...
        drawingNodes = (
          <>
            {skyStage(primary, pSkyClass, pScale)}
            {skyStage(secondary, secCls, sScale)}
          </>
        );
      }
    }
      ...
      >
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: "linear-gradient(to bottom, #87ceeb, #5cbf8a)",
            zIndex: 0,
          }}
        />
        {bgSrc && (
        ...
        <div className="book-volume">
          ...
          <div className="book-pages">
            ...
            <div className="paging-controls">
              <button
                className={`page-btn ${currentPageIndex === 0 ? "disabled" : ""}`}
                onClick={handlePrev}
              ...
              <button
                className={`page-btn ${currentPageIndex === book.pages.length - 1 ? "disabled" : ""}`}
                onClick={handleNext}
          ...
          >
            <div
              className="cover-front flat-vector"
              style={{ backgroundColor: styleConf.bg }}
            >
              <div
                className="book-binding"
                style={{ backgroundColor: styleConf.spine }}
              ...
              <div
                className="book-decoration"
                style={{ fontSize: "5rem", color: styleConf.shapeColor }}
              >
                {styleConf.shape}
              </div>
            ...
            <div
              className="cover-inside flat-inside"
              style={{ padding: 0, background: "#e0f2fe" }}
            >
              {renderLeftPage()}
            </div>
          </motion.div>
    ...
    <div className="decorations-layer">
      ...
      >
        <svg viewBox="0 0 100 100" width="130" height="130">
          ...
          <path
            d="M 50 52 Q 50 57 55 57"
            stroke="#4a3b32"
            strokeWidth="1.5"
            fill="none"
          />
          <path
            d="M 50 52 Q 50 57 45 57"
            stroke="#4a3b32"
            strokeWidth="1.5"
            fill="none"
          />
      ...
      >
        <svg viewBox="0 0 100 100" width="90" height="90">
          ...
          <g clipPath="url(#ball-clip)">
            ...
            <path
              d="M 5 20 Q 50 50 95 20 L 95 80 Q 50 50 5 80 Z"
              fill="#5eb4d7"
            />
            <path
              d="M 0 35 Q 50 65 100 35 L 100 45 Q 50 75 0 45 Z"
              fill="#fccd2a"
            />
          </g>
          <circle
      ...
      >
        <svg viewBox="0 0 100 100" width="110" height="110">
          ...
          <path
            d="M 50 10 Q 20 40 30 85 L 70 85 Q 80 40 50 10"
            fill="#fdfdfd"
          />
          <path
            d="M 50 10 Q 38 25 35 35 L 65 35 Q 62 25 50 10"
            fill="#ea4435"
          />
          <circle
            cx="50"
      ...
      <motion.div
        className="toy toy-blocks"
        animate={{ rotate: [0, -2, 2, 0] }}
        transition={{ duration: 6, repeat: Infinity }}
      >
        <svg viewBox="0 0 100 100" width="120" height="120">
          <path
            d="M 35 60 L 65 60 L 65 90 L 55 90 L 55 80 A 5 5 0 0 0 45 80 L 45 90 L 35 90 Z"
            fill="#fccd2a"
          />
          <path
            d="M 70 70 L 100 70 L 100 90 L 90 90 L 90 80 A 5 5 0 0 0 80 80 L 80 90 L 70 90 Z"
            fill="#5eb4d7"
          />
      ...
      <motion.div
        className="toy toy-duck"
        animate={{ y: [0, -3, 0], rotate: [0, -3, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
      >
        <svg viewBox="0 0 100 100" width="100" height="100">
          <path
            d="M 20 70 Q 20 90 50 90 Q 80 90 85 75 Q 90 60 70 60 Q 60 60 55 50 Q 50 65 20 70"
            fill="#fccd2a"
          />
          <path
            d="M 40 70 Q 50 80 65 70 Q 60 65 50 65 Q 40 65 40 70"
            fill="#e5b217"
          />
...
function BookshelfApp() {
  ...
  useEffect(() => {
    ...
    if (window.SERVER_DATA && window.SERVER_DATA.stories) {
      const mappedStories = window.SERVER_DATA.stories.map((s) => {
        ...
        try {
          parsedScenes =
            typeof s.scene_data === "string"
              ? JSON.parse(s.scene_data)
              : s.scene_data || [];
        } catch (e) {}
        ...
        try {
          parsedLayout =
            typeof s.layout_data === "string"
              ? JSON.parse(s.layout_data)
              : s.layout_data || null;
        } catch (e) {}
        ...
        if (parsedLayout && parsedLayout.spreads) {
          ...
          if (s.moral)
            mappedPages.push({ page: 2, content_kr: `💡 교훈:\n${s.moral}` });
        }

        // 표지 이미지는 첫 번째 장면의 병합 이미지를 우선적으로 사용
        const coverImg =
          parsedScenes && parsedScenes[0] && parsedScenes[0].merged_image
  ...
  const markAsRead = async (id) => {
    setBooks((prevBooks) =>
      prevBooks.map((b) => (b.id === id ? { ...b, isRead: true } : b)),
    );
    try {
  ...
  if (isLoading)
    return (
      <div style={{ textAlign: "center", marginTop: "3rem", color: "#d68b39" }}>
        도서관을 불러오는 중입니다...
      </div>
    );
    ...
    <div className="bookshelf-container flat-vector-bg">
      ...
      {(() => {
        ...
        const progress = info.next
          ? Math.min(
              100,
              Math.round(((totalRead - prevAt) / (nextAt - prevAt)) * 100),
            )
          : 100;
        return (
          <motion.div
          ...
          >
            ...
            <div className="level-card-info">
              ...
              <div className="level-card-sub">
                읽은 책 <strong>{totalRead}</strong>권
              </div>
              {info.next && (
                <div className="level-progress-wrap">
                  ...
                  <span className="level-progress-label">
                    {info.next}까지 {nextAt - totalRead}권 남음
                  </span>
                </div>
              )}
              {!info.next && (
                <div className="level-card-sub" style={{ color: info.color }}>
                  최고 레벨 달성! 🎉
                </div>
              )}
            </div>
      ...
      <AnimatePresence>
        {activeBook && (
          <StoryModal
            book={activeBook}
            idx={books.findIndex((b) => b.id === activeBook.id)}
            onClose={() => setActiveBook(null)}
            onReadComplete={markAsRead}
          />
        )}
        {isArchiveOpen && (
          <ArchiveModal
        ...
        {isUnreadArchiveOpen && (
          <ArchiveModal
            books={archivedUnreadBooks}
            onClose={() => setIsUnreadArchiveOpen(false)}
            onBookClick={setActiveBook}
            onDelete={deleteBook}
          />
        )}
